...
MAX_LEN = 3  # segundos
BATCH_SIZE = 32 #Número de muestras de entrenamiento procesadas antes de actualizar los pesos (balance memoria y gradiente)
...
class AudioProcessor:
    ...
    Clase encargada del procesamiento de audio crudo (forma de onda)
        convirtiéndolo en un espectrograma de Mel normalizado en dB.
    ...
    #Cambio usando pytorch para hacer el espectrograma de Mel en vez de librosa
    def __init__(self, sample_rate=22050, n_mels=128):
        self.sample_rate = sample_rate
        self.n_mels = n_mels
        #Transformar ondas a un espectograma
        self.mel_transform = torchaudio.transforms.MelSpectrogram( #Transformación para convertir auido en espectrogramas de Mel
            sample_rate=sample_rate,
            n_fft=2048, #Número de puntos para la FFT n_fft/sample_rate, resolución de la frecuencia
            hop_length=512, #Pasos
            n_mels=n_mels,
            power=2 #Espectrograma de potencia
        )
    ...
    def process(self, waveform, sample_rate):
        ...
        mel = self.mel_transform(waveform) #Convierte a espectrograma de MEL, STFT, [1, n_mels, time]
        mel_db = self.db_transform(mel) #Convierte a escala log (dB)
        mel_db = (mel_db - mel_db.mean()) / (mel_db.std() + 1e-8) #Estandarizar los valores, retorna el especrtograma normalizado 
...
class InstrumentCNN(nn.Module):
    """CNN para clasificación de espectrogramas de Mel.
...
class TrainingVisualizer:
    ...
    def _plot_metrics(self):
        ...
        plt.tight_layout()
        plt.savefig('training_metricsmel.png')
        plt.close()
    ...
    def _plot_confusion_matrix(self, model, loader):
        ...
        plt.tight_layout()
        plt.savefig('confusion_matrixmel.png')
        plt.close()
