...
# Configuración
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu") #Usar GPU o CPU
SAMPLE_RATE = 22050 #Frecuencia de muestreo en kHz
MAX_LEN = 3  # segundos
BATCH_SIZE = 32 #Npumero de muestras de entrenamiento procesadas antes de actualizar los pesos
...
class AudioProcessor:
    ...
    """
    def __init__(self, sample_rate=22050, n_mels=128): #Frecuencia de muestreo & número de bandas de frecuencia Mel
        self.sample_rate = sample_rate #Guardar la frecuencia de muestreo como atributo de clase
        self.n_mels = n_mels #Guardar las bandas MEL como atributo de clase
        ...
        self.mel_transform = torchaudio.transforms.MelSpectrogram( #Transformación para convertir auido en espectrogramas de Mel
            sample_rate=sample_rate, #La frecuencia que se volvió atributo de clase
            n_fft=2048, #Número de puntos para la FFT n_fft/sample_rate
...
class InstrumentCNN(nn.Module): #Hereda de nn.Module
    ...
    def forward(self, x):
        ...
        """
        if x.dim() == 3: #Si la entrada es de 3 dimensiones [batch, n_mels, time] 
            x = x.unsqueeze(1) #pasa a [batch, 1, n_mels, time]
        x = self.features(x) #Capas convolucionales
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, label_map):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader: #De CPU a GPU
            ...
            outputs = model(inputs) #CCalcular la salida
